...
class User < ActiveRecord::Base
  ...
  def self.load_seeds
    if User.where(email: 'admin@ramisn.com').empty?
      user = User.new(email: 'admin@ramisn.com', password: 'testing', first_name: 'Admin', last_name: 'User')
      user.account = Account.find_by_company('RSC')
      user.roles = [:superadmin]
      user.view_overlays << :geofences
      user.save!
    end
  end
